...
def rootTreeToDataFrame():
    ...
    fname_p = "LArRecoND_1_999_Partially_Cheated.root"

    #load file
    file_f = uproot.open(fname_f)
    file_p = uproot.open(fname_p)

    #get tree from file
    t_f = file_f["LArRecoND"]
    t_p = file_p["LArRecoND"]
    ...
    data_f = t_f.arrays(branches, library="ak")
    data_p = t_p.arrays(branches, library="ak")

    e_color="skyblue"
    pi_color="hotpink"

    big_data= [data_f, data_p]
    mc_e_theta_list = []
    mc_e_phi_list = []
    mc_e_rho_list = []
    mc_pi_theta_list = []
    mc_pi_phi_list = []
    mc_pi_rho_list = []
    titles = ["fully cheated", "partially cheated"]
    ...
    for i in range(0,2):
        ax = axs[i]
        n_e = len(mc_e_theta_list[i])
        n_pi = len(mc_pi_theta_list[i])
        ax.hist(mc_e_theta_list[i], bins=50, histtype="stepfilled", linewidth=2, density=True, edgecolor=e_color, alpha = 0.3,  facecolor=e_color, label=f"Electron (n={n_e})")
        ax.hist(mc_pi_theta_list[i], bins=50, histtype="stepfilled",linewidth=2, density=True, edgecolor= pi_color, alpha= 0.3,facecolor=pi_color, label=f"Pion (n={n_pi})")
        ax.hist(mc_e_theta_list[i], bins=50, histtype="stepfilled", linewidth=2, density=True, edgecolor=e_color, alpha = 1,  facecolor='none')
        ax.hist(mc_pi_theta_list[i], bins=50, histtype="stepfilled",linewidth=2, density=True, edgecolor= pi_color, alpha=1,facecolor='none')
    ...
    plt.suptitle(r"Truth $\theta$ for e and $\pi$", fontsize=16)
    ...
    fig, ax = plt.subplots(figsize=(8, 6), layout="constrained")
    ax.hist(mc_e_rho_numpy, bins=bins, histtype="step",linewidth=2,density=True, label="Electron")
    ax.hist(mc_pi_rho_numpy, bins=bins, histtype="step",linewidth=2,density=True, label="Pion")
